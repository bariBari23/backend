package store.baribari.demo.domain.order.entity
    ...
    companion object {
        fun createOrderItem(
            ...
            return OrderItem(
                ...
                status = determineOrderStatus(payMethod),
            )
        }

        private fun determineOrderStatus(payMethod: PayMethod): OrderStatus =
            if (payMethod == PayMethod.CASH || payMethod == PayMethod.BANK_TRANSFER) {
                OrderStatus.READY
            } else {
                OrderStatus.ORDERED
            }
