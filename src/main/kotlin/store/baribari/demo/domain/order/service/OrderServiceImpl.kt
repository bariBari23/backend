...
import store.baribari.demo.domain.order.entity.OrderItem.Companion.createOrderItem
import store.baribari.demo.domain.order.repository.OrderItemRepository
import store.baribari.demo.domain.order.repository.OrderRepository
...
class OrderServiceImpl(
    ...
    override fun findOneOrder(
        ...
        val order =
            orderRepository.findByIdFetchUserAndOrderItemAndDosirak(orderId)
    ...
    override fun createOrder(
        ...
        val user =
            userRepository.findByEmailFetchCart(userEmail)
                ?: throw EntityNotFoundException("$userEmail 이라는 유저는 존재하지 않습니다.")
    ...
    override fun cancelOrderItem(
        ...
        val orderItem =
            order.orderItemList.find { it.id == orderItemId }
    ...
    @Transactional
    override fun orderedOrder(orderId: Long): FindOneOrderResponseDto {
    ...
    override fun orderedOrderItem(orderItemId: Long): OrderItemDto {
        // TODO: 여기 아래부터는 상점이나 관리자만 할 수 있도록
        val orderItem =
            orderItemRepository.findByIdOrNull(orderItemId)
                ?: throw EntityNotFoundException("해당 주문 아이템이 존재하지 않습니다.")
    ...
    @Transactional
    override fun completeOrder(orderId: Long): FindOneOrderResponseDto {
        val order =
            orderRepository.findByIdFetchUserAndOrderItemAndDosirak(orderId)
                ?: throw EntityNotFoundException("해당 주문이 존재하지 않습니다.")
    ...
    @Transactional
    override fun completeOrderItem(orderItem: Long): OrderItemDto {
        val orderItems =
            orderItemRepository.findByIdOrNull(orderItem)
                ?: throw EntityNotFoundException("해당 주문 아이템이 존재하지 않습니다.")
    ...
    @Transactional
    override fun forcePickupOrder(orderId: Long): Long {
        val order =
            orderRepository.findByIdFetchUserAndOrderItemAndDosirak(orderId)
                ?: throw EntityNotFoundException("해당 주문이 존재하지 않습니다.")
    ...
    @Transactional
    override fun forcePickUpOrderItem(orderItemId: Long): Long {
        val orderItem =
            orderItemRepository.findByIdOrNull(orderItemId)
                ?: throw EntityNotFoundException("해당 주문 아이템이 존재하지 않습니다.")
