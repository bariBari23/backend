...
import store.baribari.demo.model.QLikeStore.likeStore
import store.baribari.demo.model.User
import store.baribari.demo.model.menu.Dosirak
import store.baribari.demo.model.menu.QBanchanDosirak.banchanDosirak
import store.baribari.demo.model.menu.QDosirak.dosirak
...
class DosirakRepositoryCustomImpl(
    ...
    override fun customFindDosirakByQuery(
        ...
        val dosiraks = queryFactory
            .selectFrom(dosirak)
            .leftJoin(dosirak.store).fetchJoin()
            .where(
    ...
    private fun likeStoreIds(user: User) =
        queryFactory.select(likeStore.store.id)
            .distinct()
            .from(likeStore)
    ...
    private fun usernameEq(user: User) =
        likeStore.user.email.eq(user.email)
